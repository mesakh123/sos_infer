...
class EventService:
    ...
    async def createEvent(request: EventSchema):
        ...
        timestamps = str(datetime.datetime.now(pytz.timezone(
    ...
    async def updateEvent(id: int, request: EventPartialSchema):
        ...
        filtered_req['payload_length'] = "%05d" % (
            total + 4 - len(str(row['sent'])) - len(str(row['id'])))
    ...
    async def webSocketCreateEvent(request: EventSchema):
        ...
        try:
            ...
            logger.info("Data saved at " + str(datetime.now(pytz.timezone(
                'Asia/Taipei')).strftime('%Y-%m-%d %H:%M:%S:%f')))
            return last_record_id
        except:
            logger.info("Data not saved at " + str(datetime.now(pytz.timezone(
                'Asia/Taipei')).strftime('%Y-%m-%d %H:%M:%S:%f')))
            return None
